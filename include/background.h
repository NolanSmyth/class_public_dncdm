...
struct background {
  ...
  double H0; /**< \f$ H_0 \f$: Hubble parameter (in fact, [\f$H_0/c\f$]) in \f$
                Mpc^{-1} \f$ */
  ...
  double
      Omega0_lambda; /**< \f$ \Omega_{0_\Lambda} \f$: cosmological constant */

  double Omega0_fld; /**< \f$ \Omega_{0 de} \f$: fluid */

  enum equation_of_state
      fluid_equation_of_state; /**< parametrisation scheme for fluid equation of
                                  state */

  double
      w0_fld; /**< \f$ w0_{DE} \f$: current fluid equation of state parameter */
  double wa_fld;    /**< \f$ wa_{DE} \f$: fluid equation of state parameter
                       derivative */
  ...
  double
      Omega0_ur; /**< \f$ \Omega_{0 \nu r} \f$: ultra-relativistic neutrinos */

  double
      Omega0_dcdmdr; /**< \f$ \Omega_{0 dcdm}+\Omega_{0 dr} \f$: decaying cold
                        dark matter (dcdm) decaying to dark radiation (dr) */

  double Gamma_dcdm;  /**< \f$ \Gamma_{dcdm} \f$: decay constant for decaying
                         cold dark matter */
  double Gamma_dncdm; /**< \f$ \Gamma_{dncdm} \f$: decay constant for decaying
                         non-cold dark matter */

  double Omega_ini_dcdm;  /**< \f$ \Omega_{ini,dcdm} \f$: rescaled initial value
                             for dcdm density (see 1407.2418 for definitions) */
  short has_dcdm_bg_only; /**< Flag that determines whether we only use the DCDM
                             background evolution, or both the b/g and the
                             perturbations in DCDM and DR */

  double Omega0_scf;      /**< \f$ \Omega_{0 scf} \f$: scalar field */
  short attractor_ic_scf; /**< whether the scalar field has attractor initial
                             conditions */
  double phi_ini_scf;     /**< \f$ \phi(t_0) \f$: scalar field initial value */
  double phi_prime_ini_scf; /**< \f$ d\phi(t_0)/d\tau \f$: scalar field initial
                               derivative wrt conformal time */
  double *scf_parameters;   /**< list of parameters describing the scalar field
                               potential */
  int scf_parameters_size;  /**< size of scf_parameters */
  int scf_tuning_index;     /**< index in scf_parameters used for tuning */
  // double scf_lambda; /**< \f$ \lambda \f$ : scalar field exponential
  // potential slope */ double scf_alpha;  /**< \f$ \alpha \f$ :
  // Albrecht-Skordis polynomial slope */ double scf_B; /**< \f$ \alpha \f$ :
  // Albrecht-Skordis field shift */ double scf_A; /**< \f$ \alpha \f$ :
  // Albrecht-Skordis offset
  // */
  ...
  double *Omega0_ncdm, Omega0_ncdm_tot; /**< Omega0_ncdm for each species and
                                           for the total Omega0_ncdm */
  double *deg_ncdm,
      deg_ncdm_default; /**< vector of degeneracy parameters in factor
                            of p-s-d: 1 for one family of neutrinos
                            (= one neutrino plus its anti-neutrino,
                            total g*=1+1=2, so deg = 0.5 g*); and its
                            default value */

  /* the following parameters help to define the analytical ncdm phase space
   * distributions (p-s-d) */
  ...
  double *ncdm_psd_parameters; /**< list of parameters for specifying/modifying
                                    ncdm p.s.d.'s, to be customized for given
                                  model (could be e.g. mixing angles) */
  ...
  double M_dncdm;         /**< decaying non-cold relic dimensionless ratios
                             m_dncdm/T_dncdm */
  double Omega_ini_dncdm; /**< Omega0_dncdm if the species did not decay */
  double deg_dncdm, deg_dncdm_default; /**< Degeneracy parameter in factor of
                                          p-s-d: 1 for one family of neutrinos
                                           (= one neutrino plus its
                                          anti-neutrino, total g*=1+1=2, so deg
                                          = 0.5 g*); and its default value */

  /* the following parameters help to define the analytical dncdm phase space
   * distributions (p-s-d) */
  ...
  double m_dncdm_in_eV; /**< list of ncdm masses in eV (inferred from M_ncdm and
  ...
  double K;             /**< \f$ K \f$: Curvature parameter \f$
                           K=-\Omega0_k*a_{today}^2*H_0^2\f$; */
  int sgnK;             /**< K/|K|: -1, 0 or 1 */
  double *m_ncdm_in_eV; /**< list of ncdm masses in eV (inferred from M_ncdm and
                           other parameters above) */
  double Neff; /**< so-called "effective neutrino number", computed at earliest
                  time in interpolation table */
  ...
  /** @name - all indices for the vector of background (=bg) quantities stored
   * in table */
  ...
  // Currently have two sets of indices for Decaying NCDM: for quantities
  // computed from phase space, and for quantities computed by integrating the
  // boltzmann equation for the energy density. The latter has the decay term,
  ...
  int index_bg_pseudo_p_dncdm; /**< another statistical momentum useful in ncdma
                                  approximation from 1104.2935*/
  int index_bg_lnf_dncdm;      /**< index of the first q-bin of logarithm of the
                                  q-dependent phase space distribution of the DNCDM
                                */
  int index_bg_dlnf_dlnq_dncdm; /**< index of the first q-bin of dlnf/dlnq of
                                   the DNCDM, evaluated on the bg q-grid */
  int index_bg_lnf_pt_dncdm; /**< index of the first q-bin b/g lnf interpolated
                                over the perturbation q-grid */
  int index_bg_dlnf_dlnq_pt_dncdm; /**< index of the first q-bin of dlnf/dlnq of
                                      the DNCDM, evaluated on the perturbation
                                      q-grid */

  int index_bg_Omega_r; /**< relativistic density fraction (\f$ \Omega_{\gamma}
                           + \Omega_{\nu r} \f$) */

  /* end of vector in normal format, now quantities in long format */

  int index_bg_rho_crit; /**< critical density */
  int index_bg_Omega_m;  /**< non-relativistic density fraction (\f$ \Omega_b +
                            \Omega_cdm + \Omega_{\nu nr} \f$) */
  ...
  double *tau_table; /**< vector tau_table[index_tau] with values of \f$ \tau
                        \f$ (conformal time) */
  double *z_table;   /**< vector z_table[index_tau] with values of \f$ z \f$
                        (redshift) */
  double *
      background_table; /**< table
                           background_table[index_tau*pba->bg_size+pba->index_bg]
  ...
  double *d2tau_dz2_table; /**< vector d2tau_dz2_table[index_tau] with values of
                              \f$ d^2 \tau / dz^2 \f$ (conformal time) */
  double *
      d2background_dtau2_table; /**< table
                                   d2background_dtau2_table[index_tau*pba->bg_size+pba->index_bg]
                                   with values of \f$ d^2 b_i / d\tau^2 \f$
                                   (conformal time) */

  //@}

  /** @name - all indices for the vector of background quantities to be
   * integrated (=bi)
   *
   ...
   * {B} quantities are needed by background_functions() while {C} quantities
   * are not.
   */
  ...
  int index_bi_lnf_dncdm; /**< {B} index of the first q-bin of logarithm of the
                             q-dependent phase space distribution of the DNCDM
                           */
  int index_bi_dlnf_dlnq_dncdm; /**< {B} index of the first q-bin of d lnf/dlnq
                                   of the q-dependent phase space distribution
                                   of the DNCDM */
  int index_bi_rho_dr;          /**< {B} dr density */
  int index_bi_rho_fld;         /**< {B} fluid density */
  int index_bi_phi_scf;         /**< {B} scalar field value */
  int index_bi_phi_prime_scf;   /**< {B} scalar field derivative wrt conformal
                                   time */
  ...
  int index_bi_D;       /**< {C} scale independent growth factor D(a) for CDM
                           perturbations. */
  int index_bi_D_prime; /**< {C} D satisfies \f$ [D''(\tau)=-aHD'(\tau)+3/2 a^2
                           \rho_M D(\tau) \f$ */
  ...
  /**
   *@name - arrays related to sampling and integration of ncdm phase space
   *distributions
   */

  //@{
  int *ncdm_quadrature_strategy; /**< Vector of integers according to quadrature
                                    strategy. */
  ...
  double *
      *w_ncdm; /**< Pointers to vectors of corresponding quadrature weights w */
  double **dlnf0_dlnq_ncdm; /**< Pointers to vectors of logarithmic derivatives
                               of p-s-d */
  ...
  /**
   *@name - arrays related to sampling and integration of decaying ncdm phase
   space distributions. Compared to the NCDM case, we consider only a single
   species, so the quantities have one less dimension (i.e. no index over
   flavors)
   */

  //@{
  int dncdm_quadrature_strategy; /**< Vector of integers according to quadrature
                                    strategy. */
  ...
  double *w_dncdm_bg; /**< Pointers to vector of corresponding quadrature
                         weights w */
  double *q_dncdm;    /**< Pointers to vector of perturbation sampling in q */
  double
      *w_dncdm; /**< Pointers to vector of corresponding quadrature weights w */
  double *diff_mat_dncdm_bg; /**< Pointer to the differentiation matrix used to
                                compute df/dq */
  double *dlnf0_dlnq_dncdm;  /**< Pointers to vectors of logarithmic derivatives
                                of p-s-d */
  int q_size_dncdm_bg;       /**< Size of the q_ncdm_bg array */
  int q_size_dncdm;          /**< Size of the q_ncdm array */
  int num_dr_collision_integrals; /**< Number of source terms for the DR
                                     Boltzmann hierarchy to compute */
  double factor_dncdm; /**< Normalization factor for calculating energy density
                          etc.*/
  ...
  short short_info;  /**< flag for calling background_at_eta and return little
                        information */
  short normal_info; /**< flag for calling background_at_eta and return medium
                        information */
  ...
  short background_verbose; /**< flag regulating the amount of information sent
...
/**
 * temporary parameters and workspace passed to phase space distribution
 * function
 */
...
int background_at_tau(struct background *pba, double tau, short return_format,
                      short inter_mode, int *last_index, double *pvecback);

int background_tau_of_z(struct background *pba, double z, double *tau);

int background_functions(struct background *pba, double *pvecback_B,
                         short return_format, double *pvecback);
...
int background_ncdm_distribution(void *pba, double q, double *f0);

int background_dncdm_distribution(void *pba, double q, int der, double *f0);

int background_dncdm_distribution_original(void *pba, double q, double *f0);

int background_ncdm_test_function(void *pba, double q, double *test);
...
int background_dncdm_momenta(double *lnfvec, double *qvec, double *wvec,
                             int qsize, double M, double factor, double z,
                             double *n, double *rho, double *p,
                             double *pseudo_p);

int background_ncdm_momenta(double *qvec, double *wvec, int qsize, double M,
                            double factor, double z, double *n, double *rho,
                            double *p, double *drho_dM, double *pseudo_p);
...
int background_initial_conditions(struct precision *ppr, struct background *pba,
                                  double *pvecback,
...
int background_derivs(double z, double *y, double *dy,
...
/**
 * @name Some conversion factors and fundamental constants needed by background
 * module:
 */
...
#define _Mpc_over_m_                                                           \
  3.085677581282e22 /**< conversion factor from meters to megaparsecs */
/* remark: CAMB uses 3.085678e22: good to know if you want to compare  with high
 * accuracy */
...
#define _H0_BIG_                                                               \
  1. / 2997.9 /**< maximal \f$ H_0 \f$ in \f$ Mpc^{-1} (h=1.0) \f$ */
#define _H0_SMALL_                                                             \
  0.3 / 2997.9           /**< minimal \f$ H_0 \f$ in \f$ Mpc^{-1} (h=0.3) \f$ */
#define _TCMB_BIG_ 2.8   /**< maximal \f$ T_{cmb} \f$ in K */
#define _TCMB_SMALL_ 2.7 /**< minimal \f$ T_{cmb}  \f$ in K */
#define _TOLERANCE_ON_CURVATURE_                                               \
  1.e-5 /**< if \f$ | \Omega_k | \f$ smaller than this, considered as flat */
...
#define _PSD_DERIVATIVE_EXP_MIN_                                               \
  -30 /**< for ncdm, for accurate computation of dlnf0/dlnq, q step is varied  \
         in range specified by these parameters */
#define _PSD_DERIVATIVE_EXP_MAX_                                               \
  2 /**< for ncdm, for accurate computation of dlnf0/dlnq, q step is varied in \
       range specified by these parameters */

#define _zeta3_                                                                \
  1.2020569031595942853997381615114499907649862923404988817922 /**< for        \
                                                                  quandrature  \
                                                                  test         \
                                                                  function */
#define _zeta5_                                                                \
  1.0369277551433699263313654864570341680570809195019128119741 /**< for        \
                                                                  quandrature  \
                                                                  test         \
                                                                  function */
