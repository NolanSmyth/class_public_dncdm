...
int input_init_from_arguments(int argc, char **argv, struct precision *ppr,
                              struct background *pba, struct thermo *pth,
                              struct perturbs *ppt, struct transfers *ptr,
                              struct primordial *ppm, struct spectra *psp,
                              struct nonlinear *pnl, struct lensing *ple,
  ...
  struct file_content fc_precision; /** - --> a temporary structure with all
                                       precision parameters */
  struct file_content
      fc_root; /** - --> a temporary structure with only the root name */
  struct file_content fc_inputroot; /** - --> sum of fc_inoput and fc_root */
  struct file_content
      *pfc_input; /** - --> a pointer to either fc_root or fc_inputroot */

  char input_file[_ARGUMENT_LENGTH_MAX_];
  char precision_file[_ARGUMENT_LENGTH_MAX_];
  char tmp_file[_ARGUMENT_LENGTH_MAX_ +
                26]; // 26 is enough to extend the file name [...] with the
                     // characters "output/[...]%02d_parameters.ini" (as done
                     // below)
  ...
  if (argc > 1) {
    for (i = 1; i < argc; i++) {
      ...
      if (strcmp(extension, ".ini") == 0) {
        class_test(input_file[0] != '\0', errmsg,
                   "You have passed more than one input file with extension "
                   "'.ini', choose one.");
        ...
        fprintf(stdout,
                "Warning: the file %s has an extension different from .ini and "
                ".pre, so it has been ignored\n",
                argv[i]);
      }
    }
  }
  ...
  if (input_file[0] != '\0') {
    ...
    class_call(
        parser_read_string(&fc_input, "root", &stringoutput, &flag1, errmsg),
        errmsg, errmsg);
    ...
    if (flag1 == _FALSE_) {
      ...
      class_call(parser_init(&fc_root, 1, fc_input.filename, errmsg), errmsg,
                 errmsg);
      sprintf(fc_root.name[0], "root");
      sprintf(fc_root.value[0], "output/%s%02d_", inifilename, filenum);
      fc_root.read[0] = _FALSE_;
      class_call(parser_cat(&fc_input, &fc_root, &fc_inputroot, errmsg), errmsg,
                 errmsg);
  ...
  class_call(
      input_init(&fc, ppr, pba, pth, ppt, ptr, ppm, psp, pnl, ple, pop, errmsg),
      errmsg, errmsg);
 ...
 *
 */
int input_init(struct file_content *pfc, struct precision *ppr,
               struct background *pba, struct thermo *pth, struct perturbs *ppt,
               struct transfers *ptr, struct primordial *ppm,
               struct spectra *psp, struct nonlinear *pnl, struct lensing *ple,
               struct output *pop, ErrorMsg errmsg) {
  ...
  for (index_target = 0; index_target < _NUM_TARGETS_; index_target++) {
    class_call(parser_read_double(pfc, target_namestrings[index_target],
                                  &param1, &flag1, errmsg),
               errmsg, errmsg);
    if (flag1 == _TRUE_) {
      /** - --> input_auxillary_target_conditions() takes care of the case where
         for instance Omega_dcdmdr is set to 0.0.
       */
      class_call(input_auxillary_target_conditions(pfc, index_target, param1,
                                                   &aux_flag, errmsg),
                 errmsg, errmsg);
      if (aux_flag == _TRUE_) {
        ...
        fzw.required_computation_stage =
            MAX(fzw.required_computation_stage, target_cs[index_target]);
  ...
  if (unknown_parameters_size > 0) {
    ...
    class_call(parser_init(&(fzw.fc), pfc->size + unknown_parameters_size,
                           pfc->filename, errmsg),
               errmsg, errmsg);
    ...
    class_alloc(unknown_parameter, unknown_parameters_size * sizeof(double),
                errmsg);
    class_alloc(fzw.unknown_parameters_index,
                unknown_parameters_size * sizeof(int), errmsg);
    fzw.target_size = unknown_parameters_size;
    class_alloc(fzw.target_name, fzw.target_size * sizeof(enum target_names),
                errmsg);
    class_alloc(fzw.target_value, fzw.target_size * sizeof(double), errmsg);
    ...
    for (counter = 0; counter < unknown_parameters_size; counter++) {
      ...
      strcpy(fzw.fc.name[fzw.unknown_parameters_index[counter]],
    ...
    if (unknown_parameters_size == 1) {
      ...
      /* If shooting fails, postpone error to background module to play nice
       * with MontePython. */
      class_call_try(input_find_root(&xzero, &fevals, &fzw, errmsg), errmsg,
                     pba->shooting_error, shooting_failed = _TRUE_);
      ...
      class_alloc(x_inout, sizeof(double) * unknown_parameters_size, errmsg);
      class_alloc(dxdF, sizeof(double) * unknown_parameters_size, errmsg);
      class_call(input_get_guess(x_inout, dxdF, &fzw, errmsg), errmsg, errmsg);

      class_call_try(fzero_Newton(input_try_unknown_parameters, x_inout, dxdF,
                                  unknown_parameters_size, 1e-4, 1e-6, &fzw,
                                  &fevals, errmsg),
      ...
      /* Store xzero */
      for (counter = 0; counter < unknown_parameters_size; counter++) {
        sprintf(fzw.fc.value[fzw.unknown_parameters_index[counter]], "%e",
                x_inout[counter]);
        if (input_verbose > 0) {
          fprintf(stdout, " -> found %s = %s\n",
    ...
    class_call(input_read_parameters(&(fzw.fc), ppr, pba, pth, ppt, ptr, ppm,
                                     psp, pnl, ple, pop, errmsg),
               errmsg, errmsg);
  ...
  /** - case with no unknown parameters */
    ...
    class_call(input_read_parameters(pfc, ppr, pba, pth, ppt, ptr, ppm, psp,
                                     pnl, ple, pop, errmsg),
               errmsg, errmsg);
  }

  /** - eventually write all the read parameters in a file, unread parameters in
   * another file, and warnings about unread parameters */

  class_call(
      parser_read_string(pfc, "write parameters", &string1, &flag1, errmsg),
      errmsg, errmsg);

  if ((flag1 == _TRUE_) &&
      ((strstr(string1, "y") != NULL) || (strstr(string1, "Y") != NULL))) {
  ...
  class_call(
      parser_read_string(pfc, "write warnings", &string1, &flag1, errmsg),
      errmsg, errmsg);
  ...
  if ((flag1 == _TRUE_) &&
    ...
    for (i = 0; i < pfc->size; i++) {
      if (pfc->read[i] == _FALSE_)
        fprintf(stdout,
                "[WARNING: input line not recognized and not taken into "
                "account: '%s=%s']\n",
                pfc->name[i], pfc->value[i]);
    }
  }
...
int input_read_parameters(struct file_content *pfc, struct precision *ppr,
                          struct background *pba, struct thermo *pth,
                          struct perturbs *ppt, struct transfers *ptr,
                          struct primordial *ppm, struct spectra *psp,
                          struct nonlinear *pnl, struct lensing *ple,
  ...
  class_call(input_default_precision(ppr), errmsg, errmsg);
  ...
  class_call(parser_read_string(pfc, "gauge", &string1, &flag1, errmsg), errmsg,
             errmsg);

  if (flag1 == _TRUE_) {

    if ((strstr(string1, "newtonian") != NULL) ||
        (strstr(string1, "Newtonian") != NULL) ||
        (strstr(string1, "new") != NULL)) {
      ppt->gauge = newtonian;
    }

    if ((strstr(string1, "synchronous") != NULL) ||
        (strstr(string1, "sync") != NULL) ||
  ...
  /** - h (dimensionless) and [\f$ H_0/c\f$] in \f$ Mpc^{-1} = h / 2997.9... = h
   * * 10^5 / c \f$ */
  class_call(parser_read_double(pfc, "H0", &param1, &flag1, errmsg), errmsg,
             errmsg);
  class_call(parser_read_double(pfc, "h", &param2, &flag2, errmsg), errmsg,
             errmsg);
  ...
  class_call(parser_read_double(pfc, "T_cmb", &param1, &flag1, errmsg), errmsg,
             errmsg);
  ...
  class_test(class_at_least_two_of_three(flag1, flag2, flag3), errmsg,
             "In input file, you can only enter one of T_cmb, Omega_g or "
             "omega_g, choose one");
  ...
  if (class_none_of_three(flag1, flag2, flag3)) {
    ...
    if (flag1 == _TRUE_) {
      /** - Omega0_g = rho_g / rho_c0, each of them expressed in \f$ Kg/m/s^2
       * \f$*/
    ...
    if (flag2 == _TRUE_) {
      ...
      pba->T_cmb = pow(pba->Omega0_g *
                           (3. * _c_ * _c_ * 1.e10 * pba->h * pba->h /
                            _Mpc_over_m_ / _Mpc_over_m_ / 8. / _PI_ / _G_) /
                           (4. * sigma_B / _c_),
                       0.25);
    }
  ...
  class_call(parser_read_double(pfc, "omega_b", &param2, &flag2, errmsg),
             errmsg, errmsg);
  class_test(((flag1 == _TRUE_) && (flag2 == _TRUE_)), errmsg,
             "In input file, you can only enter one of Omega_b or omega_b, "
             "choose one");
  if (flag1 == _TRUE_)
    pba->Omega0_b = param1;
  if (flag2 == _TRUE_)
  ...
  /* (a) try to read N_ur */
  class_call(parser_read_double(pfc, "N_ur", &param1, &flag1, errmsg), errmsg,
             errmsg);

  /* these lines have been added for compatibility with deprecated syntax
   * 'N_eff' instead of 'N_ur', in the future they could be suppressed */
  class_call(parser_read_double(pfc, "N_eff", &param2, &flag2, errmsg), errmsg,
             errmsg);
  class_test((flag1 == _TRUE_) && (flag2 == _TRUE_), errmsg,
             "In input file, you can only enter one of N_eff (deprecated "
             "syntax) or N_ur (up-to-date syntax), since they botgh describe "
             "the same, i.e. the contribution ukltra-relativistic species to "
             "the effective neutrino number");
  if (flag2 == _TRUE_) {
    param1 = param2;
    flag1 = _TRUE_;
    flag2 = _FALSE_;
  ...
  class_test(class_at_least_two_of_three(flag1, flag2, flag3), errmsg,
             "In input file, you can only enter one of N_eff, Omega_ur or "
             "omega_ur, choose one");
  ...
  if (class_none_of_three(flag1, flag2, flag3)) {
    ...
    if (flag1 == _TRUE_) {
      pba->Omega0_ur =
          param1 * 7. / 8. * pow(4. / 11., 4. / 3.) * pba->Omega0_g;
    }
    if (flag2 == _TRUE_) {
  ...
  class_call(parser_read_double(pfc, "omega_cdm", &param2, &flag2, errmsg),
             errmsg, errmsg);
  class_test(((flag1 == _TRUE_) && (flag2 == _TRUE_)), errmsg,
             "In input file, you can only enter one of Omega_cdm or omega_cdm, "
             "choose one");
  if (flag1 == _TRUE_)
  ...
  class_call(parser_read_double(pfc, "omega_dcdmdr", &param2, &flag2, errmsg),
             errmsg, errmsg);
  class_test(((flag1 == _TRUE_) && (flag2 == _TRUE_)), errmsg,
             "In input file, you can only enter one of Omega_dcdmdr or "
             "omega_dcdmdr, choose one");
  if (flag1 == _TRUE_)
  ...
  if (pba->Omega0_dcdmdr > 0) {
    ...
    class_call(
        parser_read_double(pfc, "Omega_ini_dcdm", &param1, &flag1, errmsg),
        errmsg, errmsg);
    class_call(
        parser_read_double(pfc, "omega_ini_dcdm", &param2, &flag2, errmsg),
        errmsg, errmsg);
    class_test(((flag1 == _TRUE_) && (flag2 == _TRUE_)), errmsg,
               "In input file, you can only enter one of Omega_ini_dcdm or "
               "omega_ini_dcdm, choose one");
    if (flag1 == _TRUE_)
    ...
    /** - Read in flag that determines whether or not we include evolution of
     * DCDM/DR perturbations or just the b/g */
    class_call(
        parser_read_string(pfc, "dcdm_bg_only", &string1, &flag1, errmsg),
        errmsg, errmsg);
    if (flag1 == _TRUE_) {
  ...
  if (flag1 == _TRUE_ && param1 > 0) {
     ...
     * e.g. 1 real scalar has 1 d.o.f but degeneracy parameter 0.5 (see the def.
     * of the psd in background.c).
     * */
    class_read_double_or_default("deg_dncdm", pba->deg_dncdm,
                                 pba->deg_dncdm_default);

    class_read_int_or_default("q_size_dncdm_bg", pba->q_size_dncdm_bg,
                              pba->q_size_dncdm_bg);
    class_read_int_or_default("q_size_dncdm", pba->q_size_dncdm,
                              pba->q_size_dncdm);

    class_read_int_or_default("num_dr_collision_integrals",
                              pba->num_dr_collision_integrals,
                              pba->num_dr_collision_integrals);
    if (input_verbose > 0) {
      printf(" -> Read in Decaying NCDM parameters: Omega_ini_dncdm, m_dncdm, "
             "dofs, Gamma_dncdm = %e, %e, %e, %e\n",
             pba->Omega_ini_dncdm, pba->m_dncdm_in_eV, 2. * pba->deg_dncdm,
             pba->Gamma_dncdm);
      printf(" -> Read in Decaying NCDM parameters: q_size_dncdm_bg, "
             "q_size_dncdm = %d, %d\n",
             pba->q_size_dncdm_bg, pba->q_size_dncdm);
    }

    /* Given the degeneracy parameter and initial energy density we can solve
     * for the temperature,
     * assuming the Decaying NCDM is thermal; This assumes that DNCDM is a
     * fermion */
    pba->T_dncdm =
        pow((8. / 7.) * pba->Omega_ini_dncdm / pba->Omega0_g / pba->deg_dncdm,
            0.25);
    if (input_verbose > 0)
      printf(" -> Evaluated decaying NCDM temperature, relative to photons to "
             "be T_dncdm = %e\n",
             pba->T_dncdm);
    ...
    class_read_int_or_default("Quadrature strategy",
                              pba->dncdm_quadrature_strategy, 0);
    /* Number of momentum bins */
    class_read_int_or_default("Number of momentum bins",
                              pba->dncdm_input_q_size, -1);

    /* qmax, if relevant */
    class_read_double_or_default("Maximum q", pba->dncdm_qmax, 15);

    /* Read chemical potentials: */
    class_read_double_or_default("ksi_dncdm", pba->ksi_dncdm,
                                 pba->ksi_dncdm_default);

    /* Read (optional) p.s.d.-parameters:*/
    parser_read_list_of_doubles(pfc, "dncdm_psd_parameters", &entries_read,
                                &(pba->dncdm_psd_parameters), &flag2, errmsg);

    class_call(background_dncdm_init(ppr, pba), pba->error_message, errmsg);

    // We do not need to compute the Decayed NCDM density today, since it has
    // decayed
    pba->M_dncdm =
        pba->m_dncdm_in_eV / _k_B_ * _eV_ / pba->T_dncdm / pba->T_cmb;
  }
  ...
  if ((flag1 == _TRUE_) && (N_ncdm > 0)) {
    pba->N_ncdm = N_ncdm;
    /* Precision parameters for ncdm has to be read now since they are used
     * here:*/
    class_read_double("tol_M_ncdm", ppr->tol_M_ncdm);
    class_read_double("tol_ncdm_newtonian", ppr->tol_ncdm_newtonian);
    class_read_double("tol_ncdm_synchronous", ppr->tol_ncdm_synchronous);
    ...
    /* Quadrature modes, 0 is qm_auto. */
    class_read_list_of_integers_or_default(
        "Quadrature strategy", pba->ncdm_quadrature_strategy, 0, N_ncdm);
    /* Number of momentum bins */
    class_read_list_of_integers_or_default("Number of momentum bins",
                                           pba->ncdm_input_q_size, -1, N_ncdm);

    /* qmax, if relevant */
    class_read_list_of_doubles_or_default("Maximum q", pba->ncdm_qmax, 15,
                                          N_ncdm);

    /* Read temperatures: */
    class_read_list_of_doubles_or_default("T_ncdm", pba->T_ncdm,
                                          pba->T_ncdm_default, N_ncdm);

    /* Read chemical potentials: */
    class_read_list_of_doubles_or_default("ksi_ncdm", pba->ksi_ncdm,
                                          pba->ksi_ncdm_default, N_ncdm);

    /* Read degeneracy of each ncdm species: */
    class_read_list_of_doubles_or_default("deg_ncdm", pba->deg_ncdm,
                                          pba->deg_ncdm_default, N_ncdm);
    ...
    for (n = 0; n < N_ncdm; n++) {
      ...
      if (pba->M_ncdm[n] != 0.0) {
        class_test(pba->Omega0_ncdm[n] != 0, errmsg,
                   "Nonzero values for both Omega and omega for ncdm species "
                   "%d are specified!",
                   n);
    ...
    class_read_list_of_integers_or_default("use_ncdm_psd_files", pba->got_files,
                                           _FALSE_, N_ncdm);
    ...
    if (flag1 == _TRUE_) {
      ...
      if (fileentries > 0) {
        ...
        /* Okay, read filenames.. */
        class_call(parser_read_list_of_strings(
                       pfc, "ncdm_psd_filenames", &entries_read,
                       &(pba->ncdm_psd_files), &flag2, errmsg),
                   errmsg, errmsg);
        ...
        class_test(entries_read != fileentries, errmsg,
                   "Number of filenames found, %d, does not match number of "
                   "_TRUE_ values in use_ncdm_files, %d",
                   entries_read, fileentries);
      }
    }
    /* Read (optional) p.s.d.-parameters:*/
    parser_read_list_of_doubles(pfc, "ncdm_psd_parameters", &entries_read,
                                &(pba->ncdm_psd_parameters), &flag2, errmsg);

    class_call(background_ncdm_init(ppr, pba), pba->error_message, errmsg);
    ...
    for (n = 0; n < N_ncdm; n++) {
      if (pba->m_ncdm_in_eV[n] != 0.0) {
        ...
        // printf("M_ncdm:%g\n",pba->M_ncdm[n]);
        pba->m_ncdm_in_eV[n] =
            _k_B_ / _eV_ * pba->T_ncdm[n] * pba->M_ncdm[n] * pba->T_cmb;
      }
  ...
  class_test(
      ...
      errmsg,
      "In input file, either Omega_Lambda or Omega_fld must be left "
      "unspecified, except if Omega_scf is set and <0.0, in which case the "
      "contribution from the scalar field will be the free parameter.");
  ...
  */
  ...
  class_test(((flag1 == _FALSE_) || (flag2 == _FALSE_)) &&
             ...
             errmsg,
             "It looks like you want to fulfil the closure relation sum Omega "
             "= 1 using the scalar field, so you have to specify both "
             "Omega_lambda and Omega_fld in the .ini file");
  ...
  if (pba->Omega0_fld != 0.) {
    ...
    class_call(parser_read_string(pfc, "fluid_equation_of_state", &string1,
                                  &flag1, errmsg),
               errmsg, errmsg);
    ...
    if (flag1 == _TRUE_) {
      ...
      if ((strstr(string1, "CLP") != NULL) ||
        ...
        class_stop(errmsg,
                   "incomprehensible input '%s' for the field "
  ...
  if (pba->Omega0_scf != 0.) {
    ...
    class_call(parser_read_list_of_doubles(
                   pfc, "scf_parameters", &(pba->scf_parameters_size),
                   &(pba->scf_parameters), &flag1, errmsg),
               errmsg, errmsg);
    class_read_int("scf_tuning_index", pba->scf_tuning_index);
    class_test(pba->scf_tuning_index >= pba->scf_parameters_size, errmsg,
               "Tuning index scf_tuning_index = %d is larger than the number "
               "of entries %d in scf_parameters. Check your .ini file.",
               pba->scf_tuning_index, pba->scf_parameters_size);
    /** - Assign shooting parameter */
    class_read_double("scf_shooting_parameter",
                      pba->scf_parameters[pba->scf_tuning_index]);

    scf_lambda = pba->scf_parameters[0];
    if ((fabs(scf_lambda) < 3.) && (pba->background_verbose > 1))
      printf("lambda = %e <3 won't be tracking (for exp quint) unless "
             "overwritten by tuning function\n",
             scf_lambda);

    class_call(
        parser_read_string(pfc, "attractor_ic_scf", &string1, &flag1, errmsg),
        errmsg, errmsg);
    ...
    if (flag1 == _TRUE_) {
      if ((strstr(string1, "y") != NULL) || (strstr(string1, "Y") != NULL)) {
        ...
        class_test(
            pba->scf_parameters_size < 2, errmsg,
  ...
  class_call(parser_read_string(pfc, "YHe", &string1, &flag1, errmsg), errmsg,
             errmsg);
  ...
  class_call(
      parser_read_string(pfc, "reio_parametrization", &string1, &flag1, errmsg),
      errmsg, errmsg);
  ...
  if (flag1 == _TRUE_) {
    ...
    class_test(flag2 == _FALSE_, errmsg,
               "could not identify reionization_parametrization value, check "
               "that it is one of 'reio_none', 'reio_camb', 'reio_bins_tanh', "
               "'reio_half_tanh', 'reio_many_tanh', 'reio_inter'...");
  }
  ...
  if ((pth->reio_parametrization == reio_camb) ||
    ...
    class_call(parser_read_double(pfc, "tau_reio", &param2, &flag2, errmsg),
               errmsg, errmsg);
    class_test(((flag1 == _TRUE_) && (flag2 == _TRUE_)), errmsg,
               "In input file, you can only enter one of z_reio or tau_reio, "
               "choose one");
    if (flag1 == _TRUE_) {
      pth->z_reio = param1;
    ...
    class_read_double("helium_fullreio_redshift",
  ...
  if (pth->reio_parametrization == reio_bins_tanh) {
    ...
    class_read_list_of_doubles("binned_reio_z", pth->binned_reio_z,
                               pth->binned_reio_num);
    class_read_list_of_doubles("binned_reio_xe", pth->binned_reio_xe,
                               pth->binned_reio_num);
    class_read_double("binned_reio_step_sharpness",
  ...
  if (pth->reio_parametrization == reio_many_tanh) {
    ...
    class_read_list_of_doubles("many_tanh_z", pth->many_tanh_z,
                               pth->many_tanh_num);
    class_read_list_of_doubles("many_tanh_xe", pth->many_tanh_xe,
                               pth->many_tanh_num);
  ...
  if (pth->reio_parametrization == reio_inter) {
    ...
    class_read_list_of_doubles("reio_inter_z", pth->reio_inter_z,
                               pth->reio_inter_num);
    class_read_list_of_doubles("reio_inter_xe", pth->reio_inter_xe,
                               pth->reio_inter_num);
  }
  ...
  if (pth->annihilation > 0.) {
    ...
    class_call(
        parser_read_string(pfc, "on the spot", &(string1), &(flag1), errmsg),
        errmsg, errmsg);
  ...
  class_call(parser_read_string(pfc, "compute damping scale", &(string1),
                                &(flag1), errmsg),
             errmsg, errmsg);
  ...
  /** (c) define which perturbations and sources should be computed, and down to
   * which scale */
    ...
    if ((strstr(string1, "mPk") != NULL) || (strstr(string1, "MPk") != NULL) ||
      ...
      /*if (pba->Omega0_ncdm_tot != 0.0){
          class_call(parser_read_string(pfc,"pk_only_cdm_bar",&string1,&flag1,errmsg),
                     ...
                     errmsg);
          if (flag1 == _TRUE_){
              if((strstr(string1,"y") != NULL) || (strstr(string1,"Y") !=
      NULL)){ ppt->pk_only_cdm_bar = _TRUE_;
              }
              else {
  ...
  if (ppt->has_density_transfers == _TRUE_) {
    class_call(parser_read_string(pfc, "extra metric transfer functions",
  ...
  if (ppt->has_cl_cmb_temperature == _TRUE_) {
    ...
    class_call(parser_read_string(pfc, "temperature contributions", &string1,
                                  &flag1, errmsg),
               errmsg, errmsg);
    ...
    if (flag1 == _TRUE_) {
      ...
      if ((strstr(string1, "tsw") != NULL) || (strstr(string1, "TSW") != NULL))
        ppt->switch_sw = 1;
      if ((strstr(string1, "eisw") != NULL) ||
          (strstr(string1, "EISW") != NULL))
        ppt->switch_eisw = 1;
      if ((strstr(string1, "lisw") != NULL) ||
          (strstr(string1, "LISW") != NULL))
        ppt->switch_lisw = 1;
      ...
      class_test(
          (ppt->switch_sw == 0) && (ppt->switch_eisw == 0) &&
              (ppt->switch_lisw == 0) && (ppt->switch_dop == 0) &&
              (ppt->switch_pol == 0),
          errmsg,
          "In the field 'output', you selected CMB temperature, but in the "
          "field 'temperature contributions', you removed all contributions");
  ...
  if (ppt->has_cl_number_count == _TRUE_) {
    ...
    class_call(parser_read_string(pfc, "number count contributions", &string1,
                                  &flag1, errmsg),
               errmsg, errmsg);
    ...
    if (flag1 == _TRUE_) {
      ...
      class_test(
          (ppt->has_nc_density == _FALSE_) && (ppt->has_nc_rsd == _FALSE_) &&
  ...
  if (ppt->has_perturbations == _TRUE_) {
    ...
    class_call(parser_read_string(pfc, "perturbed recombination", &(string1),
                                  &(flag1), errmsg),
               errmsg, errmsg);
    ...
    if (flag1 == _TRUE_) {
      ...
      /* if no modes are specified, the default is has_scalars=_TRUE_;
         but if they are specified we should reset has_scalars to _FALSE_ before
         reading */
      ...
      class_test(class_none_of_three(ppt->has_scalars, ppt->has_vectors,
                                     ppt->has_tensors),
                 errmsg,
                 "You wrote: modes=%s. Could not identify any of the modes "
                 "('s', 'v', 't') in such input",
                 string1);
    }
    ...
    if (ppt->has_scalars == _TRUE_) {
      ...
      if (flag1 == _TRUE_) {
        ...
        /* if no initial conditions are specified, the default is has_ad=_TRUE_;
           but if they are specified we should reset has_ad to _FALSE_ before
           reading */
        ppt->has_ad = _FALSE_;
        ...
        if ((strstr(string1, "cdi") != NULL) ||
            (strstr(string1, "CDI") != NULL))
          ppt->has_cdi = _TRUE_;

        if ((strstr(string1, "nid") != NULL) ||
            (strstr(string1, "NID") != NULL))
          ppt->has_nid = _TRUE_;

        if ((strstr(string1, "niv") != NULL) ||
            (strstr(string1, "NIV") != NULL))
          ppt->has_niv = _TRUE_;

        class_test(ppt->has_ad == _FALSE_ && ppt->has_bi == _FALSE_ &&
                       ppt->has_cdi == _FALSE_ && ppt->has_nid == _FALSE_ &&
                       ppt->has_niv == _FALSE_,
                   errmsg,
                   "You wrote: ic=%s. Could not identify any of the initial "
                   "conditions ('ad', 'bi', 'cdi', 'nid', 'niv') in such input",
                   string1);
      }
    }
      ...
      class_test(ppt->has_cl_cmb_lensing_potential == _TRUE_, errmsg,
                 "Inconsistency: you want C_l's for cmb lensing potential, but "
                 "no scalar modes\n");

      class_test(
          ppt->has_pk_matter == _TRUE_, errmsg,
          "Inconsistency: you want P(k) of matter, but no scalar modes\n");
    }

    if (ppt->has_vectors == _TRUE_) {

      class_test((ppt->has_cl_cmb_temperature == _FALSE_) &&
                     (ppt->has_cl_cmb_polarization == _FALSE_),
                 errmsg,
                 "inconsistent input: you asked for vectors, so you should "
                 "have at least one non-zero tensor source type (temperature "
                 "or polarization). Please adjust your input.");
    }
    ...
    if (ppt->has_tensors == _TRUE_) {
      ...
      class_test((ppt->has_cl_cmb_temperature == _FALSE_) &&
                     (ppt->has_cl_cmb_polarization == _FALSE_),
                 errmsg,
                 "inconsistent input: you asked for tensors, so you should "
                 "have at least one non-zero tensor source type (temperature "
                 "or polarization). Please adjust your input.");
    }
  }
  ...
  if (flag1 == _TRUE_) {
    ...
    class_test(flag2 == _FALSE_, errmsg,
               "could not identify primordial spectrum type, check that it is "
               "one of 'analytic_pk', 'two_scales', 'inflation_V', "
               "'inflation_H', 'external_Pk'...");
  }
  ...
  if (ppm->primordial_spec_type == two_scales) {
    ...
    if (ppt->has_scalars == _TRUE_) {
      ...
      if ((ppt->has_bi == _TRUE_) || (ppt->has_cdi == _TRUE_) ||
        ...
        class_test(pii1 <= 0., errmsg,
                   "since you request iso modes, you should have P_{ii}^1 "
                   "strictly positive");
        class_test(pii2 < 0., errmsg,
                   "since you request iso modes, you should have P_{ii}^2 "
                   "positive or eventually null");
        class_test(pri2 < 0., errmsg,
                   "by definition, you should have |P_{ri}^2| positive or "
                   "eventually null");

        flag1 = _FALSE_;

        class_call(
            parser_read_string(pfc, "special iso", &string1, &flag1, errmsg),
            errmsg, errmsg);
        ...
        /* curvaton case, only one iso parameter: piir1  */
                 ...
                 (strstr(string1, "anticurvaton") != NULL)) {
          n_iso = ppm->n_s;
          n_cor = 0.;
          c_cor = 1.;
        }
        ...
        /* general case, but if pii2 or pri2=0 the code interprets it
          ...
          if (pri2 == 0.) {
            ...
            class_test((pri1 == 0.) || (pri2 <= 0.) || (pii1 * pii2 < 0),
                       errmsg, "should NEVER happen");
            n_cor = log(pri2 / fabs(pri1)) / log(k2 / k1) -
                    0.5 * (ppm->n_s + n_iso - 2.);
          }
          class_test((pii1 * prr1 <= 0.), errmsg, "should NEVER happen");
          class_test(fabs(pri1) / sqrt(pii1 * prr1) > 1, errmsg,
                     "too large ad-iso cross-correlation in k1");
          class_test(
              fabs(pri1) / sqrt(pii1 * prr1) * exp(n_cor * log(k2 / k1)) > 1,
              errmsg, "too large ad-iso cross-correlation in k2");
          c_cor =
              -pri1 / sqrt(pii1 * prr1) * exp(n_cor * log(ppm->k_pivot / k1));
        }
        /* formula for f_iso valid in all cases */
        class_test((pii1 == 0.) || (prr1 == 0.) || (pii1 * prr1 < 0.), errmsg,
                   "should NEVER happen");
        f_iso = sqrt(pii1 / prr1) *
                exp(0.5 * (n_iso - ppm->n_s) * log(ppm->k_pivot / k1));
      }
    ...
    if (ppt->has_scalars == _TRUE_) {
      ...
      class_call(parser_read_double(pfc, "A_s", &param1, &flag1, errmsg),
                 errmsg, errmsg);
      class_call(
          parser_read_double(pfc, "ln10^{10}A_s", &param2, &flag2, errmsg),
          errmsg, errmsg);
      class_test((flag1 == _TRUE_) && (flag2 == _TRUE_), errmsg,
                 "In input file, you cannot enter both A_s and ln10^{10}A_s, "
                 "choose one");
      if (flag1 == _TRUE_)
        ppm->A_s = param1;
      ...
      if ((ppt->has_ad == _TRUE_) && (ppt->has_bi == _TRUE_)) {
        ...
        class_read_double_one_of_two("alpha_ad_bi", "alpha_bi_ad",
                                     ppm->alpha_ad_bi);
      }
      ...
      if ((ppt->has_ad == _TRUE_) && (ppt->has_cdi == _TRUE_)) {
        ...
        class_read_double_one_of_two("alpha_ad_cdi", "alpha_cdi_ad",
                                     ppm->alpha_ad_cdi);
      }
      ...
      if ((ppt->has_ad == _TRUE_) && (ppt->has_nid == _TRUE_)) {
        ...
        class_read_double_one_of_two("alpha_ad_nid", "alpha_nid_ad",
                                     ppm->alpha_ad_nid);
      }
      ...
      if ((ppt->has_ad == _TRUE_) && (ppt->has_niv == _TRUE_)) {
        ...
        class_read_double_one_of_two("alpha_ad_niv", "alpha_niv_ad",
                                     ppm->alpha_ad_niv);
      }
      ...
      if ((ppt->has_bi == _TRUE_) && (ppt->has_cdi == _TRUE_)) {
        ...
        class_read_double_one_of_two("alpha_bi_cdi", "alpha_cdi_bi",
                                     ppm->alpha_bi_cdi);
      }
      ...
      if ((ppt->has_bi == _TRUE_) && (ppt->has_nid == _TRUE_)) {
        ...
        class_read_double_one_of_two("alpha_bi_nid", "alpha_nid_bi",
                                     ppm->alpha_bi_nid);
      }
      ...
      if ((ppt->has_bi == _TRUE_) && (ppt->has_niv == _TRUE_)) {
        ...
        class_read_double_one_of_two("alpha_bi_niv", "alpha_niv_bi",
                                     ppm->alpha_bi_niv);
      }
      ...
      if ((ppt->has_cdi == _TRUE_) && (ppt->has_nid == _TRUE_)) {
        ...
        class_read_double_one_of_two("alpha_cdi_nid", "alpha_nid_cdi",
                                     ppm->alpha_cdi_nid);
      }
      ...
      if ((ppt->has_cdi == _TRUE_) && (ppt->has_niv == _TRUE_)) {
        ...
        class_read_double_one_of_two("alpha_cdi_niv", "alpha_niv_cdi",
                                     ppm->alpha_cdi_niv);
      }
      ...
      if ((ppt->has_nid == _TRUE_) && (ppt->has_niv == _TRUE_)) {
        ...
        class_read_double_one_of_two("alpha_nid_niv", "alpha_niv_nid",
                                     ppm->alpha_nid_niv);
      }
    }
    ...
    if (ppt->has_tensors == _TRUE_) {
      ...
      if (ppm->r <= 0) {
        ...
        if ((flag1 == _TRUE_) && !((strstr(string1, "SCC") != NULL) ||
                                   (strstr(string1, "scc") != NULL))) {
          class_read_double("n_t", ppm->n_t);
        } else {
          /* enforce single slow-roll self-consistency condition (order 2 in
           * slow-roll) */
    ...
    if (ppm->primordial_spec_type == inflation_V) {
      ...
      if (flag1 == _TRUE_) {
        ...
        ppm->H4 =
            4. * _PI_ * HSR4 * ppm->H0 * ppm->H0 * ppm->H0 / ppm->H1 / ppm->H1;
    ...
    class_call(
        parser_read_string(pfc, "full_potential", &string1, &flag1, errmsg),
        errmsg, errmsg);
    ...
    if (flag1 == _TRUE_) {
      if (strcmp(string1, "polynomial") == 0) {
        ...
        class_stop(errmsg,
                   "did not recognize input parameter 'potential': should be "
                   "one of 'polynomial' or 'higgs_inflation'");
      }
    }
    ...
    class_test((flag1 == _TRUE_) && (flag2 == _TRUE_), errmsg,
               "In input file, you can only enter one of ln_aH_ratio or "
               "N_star, the two are not compatible");

    if (flag1 == _TRUE_) {
      if ((strstr(string1, "auto") != NULL) ||
          (strstr(string1, "AUTO") != NULL)) {
    ...
    class_call(
        parser_read_string(pfc, "inflation_behavior", &string1, &flag1, errmsg),
        errmsg, errmsg);
    ...
    if (flag1 == _TRUE_) {
      if (strstr(string1, "numerical") != NULL) {
        ...
        class_stop(
            errmsg,
            "Your entry for 'inflation behavior' could not be understood");
      }
    }
  }
    ...
    class_call(parser_read_string(pfc, "command", &(string1), &(flag1), errmsg),
               errmsg, errmsg);
    class_test(strlen(string1) == 0, errmsg,
  ...
  if ((flag1 == _TRUE_) &&
    ...
    if ((ppt->has_scalars == _TRUE_) &&
      ...
      class_stop(
          errmsg,
          "you asked for lensed CMB Cls, but this requires a minimal number of "
          "options: 'modes' should include 's', 'output' should include 'tCl' "
          "and/or 'pCL', and also, importantly, 'lCl', the CMB lensing "
          "potential spectrum. You forgot one of those in your input.");
    }
  }
  ...
  if ((ppt->has_pk_matter == _TRUE_) ||
    ...
    class_call(
        parser_read_double(pfc, "P_k_max_h/Mpc", &param1, &flag1, errmsg),
        errmsg, errmsg);
    class_call(
        parser_read_double(pfc, "P_k_max_1/Mpc", &param2, &flag2, errmsg),
        errmsg, errmsg);
    class_test((flag1 == _TRUE_) && (flag2 == _TRUE_), errmsg,
               "In input file, you cannot enter both P_k_max_h/Mpc and "
               "P_k_max_1/Mpc, choose one");
    if (flag1 == _TRUE_) {
    ...
    class_call(parser_read_list_of_doubles(pfc, "z_pk", &(int1), &(pointer1),
                                           &flag1, errmsg),
               errmsg, errmsg);

    if (flag1 == _TRUE_) {
      class_test(int1 > _Z_PK_NUM_MAX_, errmsg,
                 "you want to write some output for %d different values of z, "
                 "hence you should increase _Z_PK_NUM_MAX_ in include/output.h "
  ...
  if ((ppt->has_cl_number_count == _TRUE_) ||
    ...
    class_call(
        parser_read_string(pfc, "selection", &(string1), &(flag1), errmsg),
        errmsg, errmsg);
    ...
    if (flag1 == _TRUE_) {
      if (strstr(string1, "gaussian") != NULL) {
        ...
        class_stop(errmsg, "In selection function input: type %s is unclear",
                   string1);
      }
    }

    class_call(parser_read_list_of_doubles(pfc, "selection_mean", &(int1),
                                           &(pointer1), &flag1, errmsg),
               errmsg, errmsg);

    if ((flag1 == _TRUE_) && (int1 > 0)) {

      class_test(int1 > _SELECTION_NUM_MAX_, errmsg,
                 "you want to compute density Cl's for %d different bins, "
                 "hence you should increase _SELECTION_NUM_MAX_ in "
                 "include/transfer.h to at least this number",
                 int1);

      ppt->selection_num = int1;
      for (i = 0; i < int1; i++) {
        class_test((pointer1[i] < 0.) || (pointer1[i] > 1000.), errmsg,
                   "input of selection functions: you asked for a mean "
                   "redshift equal to %e, sounds odd",
                   pointer1[i]);
      ...
      for (i = 1; i < int1; i++) {
        class_test(ppt->selection_mean[i] <= ppt->selection_mean[i - 1], errmsg,
                   "input of selection functions: the list of mean redshifts "
                   "must be passed in growing order; you entered %e before %e",
                   ppt->selection_mean[i - 1], ppt->selection_mean[i]);
        ppt->selection_width[i] = ppt->selection_width[0];
        ptr->selection_bias[i] = ptr->selection_bias[0];
        ptr->selection_magnification_bias[i] =
            ptr->selection_magnification_bias[0];
      }

      class_call(parser_read_list_of_doubles(pfc, "selection_width", &(int1),
                                             &(pointer1), &flag1, errmsg),
                 errmsg, errmsg);
      ...
      if ((flag1 == _TRUE_) && (int1 > 0)) {
        ...
        if (int1 == 1) {
          ...
          class_stop(
              errmsg,
              "In input for selection function, you asked for %d bin centers "
              "and %d bin widths; number of bins unclear; you should pass "
              "either one bin width (common to all bins) or %d bin widths",
              ppt->selection_num, int1, ppt->selection_num);
        }
        free(pointer1);
      }

      class_call(parser_read_list_of_doubles(pfc, "selection_bias", &(int1),
                                             &(pointer1), &flag1, errmsg),
                 errmsg, errmsg);
      ...
      if ((flag1 == _TRUE_) && (int1 > 0)) {
        ...
        if (int1 == 1) {
          ...
          class_stop(
              errmsg,
              "In input for selection function, you asked for %d bin centers "
              "and %d bin biases; number of bins unclear; you should pass "
              "either one bin bias (common to all bins) or %d bin biases",
              ppt->selection_num, int1, ppt->selection_num);
        }
        free(pointer1);
      }
      ...
      class_call(
          parser_read_list_of_doubles(pfc, "selection_magnification_bias",
                                      &(int1), &(pointer1), &flag1, errmsg),
          errmsg, errmsg);
    ...
    if (ppt->selection_num > 1) {
      ...
      if ((psp->non_diag < 0) || (psp->non_diag >= ppt->selection_num))
        class_stop(errmsg,
                   "Input for non_diagonal is %d, while it is expected to be "
                   "between 0 and %d\n",
                   psp->non_diag, ppt->selection_num - 1);
    }

    class_call(
        parser_read_string(pfc, "dNdz_selection", &(string1), &(flag1), errmsg),
        errmsg, errmsg);
    ...
    class_call(
        parser_read_string(pfc, "dNdz_evolution", &(string1), &(flag1), errmsg),
        errmsg, errmsg);
    ...
    flag1 = _FALSE_;
    class_call(parser_read_double(pfc, "bias", &param1, &flag1, errmsg), errmsg,
               errmsg);
    class_test(
        flag1 == _TRUE_, errmsg,
        "the input parameter 'bias' is obsolete, because you can now pass an "
        "independent light-to-mass bias for each bin/selection function. The "
        "new input name is 'selection_bias'. It can be set to a single number "
        "(common bias for all bins) or as many numbers as the number of bins");

    flag1 = _FALSE_;
    class_call(parser_read_double(pfc, "s_bias", &param1, &flag1, errmsg),
               errmsg, errmsg);
    class_test(flag1 == _TRUE_, errmsg,
               "the input parameter 's_bias' is obsolete, because you can now "
               "pass an independent magnitude bias for each bin/selection "
               "function. The new input name is 'selection_magnitude_bias'. It "
               "can be set to a single number (common magnitude bias for all "
               "bins) or as many numbers as the number of bins");
  }
    ...
    if (flag1 == _TRUE_) {
      ...
      if ((ppt->has_cl_number_count == _TRUE_) ||
        ...
        for (bin = 0; bin < ppt->selection_num; bin++) {
          ...
          /* the few lines below should be consistent with their counterpart in
           * transfer.c, in transfer_selection_times() */
          ...
          if (ppt->selection == tophat) {
            z_max = ppt->selection_mean[bin] +
                    (1. +
                     ppr->selection_cut_at_sigma * ppr->selection_tophat_edge) *
  ...
  class_call(parser_read_string(pfc, "root", &string1, &flag1, errmsg), errmsg,
             errmsg);
  if (flag1 == _TRUE_) {
    class_test(strlen(string1) > _FILENAMESIZE_ - 32, errmsg,
               "Root directory name is too long. Please install in other "
               "directory, or increase _FILENAMESIZE_ in common.h");
  ...
  if (flag1 == _TRUE_) {
    ...
    if ((strstr(string1, "class") != NULL) ||
        (strstr(string1, "CLASS") != NULL))
      pop->output_format = class_format;
    else {
      if ((strstr(string1, "camb") != NULL) ||
          (strstr(string1, "CAMB") != NULL))
        pop->output_format = camb_format;
      else
        class_stop(errmsg,
                   "You wrote: format=%s. Could not identify any of the "
                   "possible formats ('class', 'CLASS', 'camb', 'CAMB')",
                   string1);
    }
  }
  ...
  class_call(
      parser_read_string(pfc, "non linear", &(string1), &(flag1), errmsg),
      errmsg, errmsg);

  if (flag1 == _TRUE_) {

    class_test(ppt->has_perturbations == _FALSE_, errmsg,
               "You requested non linear computation but no linear "
               "computation. You must set output to tCl or similar.");

    if ((strstr(string1, "halofit") != NULL) ||
  ...
  /** (g) amount of information sent to standard output (none if all set to
   * zero) */
  ...
  class_read_double("a_ini_over_a_today_default",
                    ppr->a_ini_over_a_today_default);
  class_read_double("back_integration_stepsize",
                    ppr->back_integration_stepsize);
  class_read_double("tol_background_integration",
  ...
  class_read_double("recfast_x_He0_trigger_delta",
  ...
  class_read_double("recfast_x_H0_trigger_delta",
  ...
  class_read_string("two_photon_tables hyrec file",
  ...
  class_read_double("reionization_optical_depth_tol",
                    ppr->reionization_optical_depth_tol);
  class_read_double("reionization_start_factor",
                    ppr->reionization_start_factor);

  class_read_int("thermo_rate_smoothing_radius",
  ...
  class_read_double("k_scalar_min_tau0",
                    ppr->k_min_tau0); // obsolete precision parameter: read for
                                      // compatibility with old precision files
  class_read_double(
      "k_scalar_max_tau0_over_l_max",
      ppr->k_max_tau0_over_l_max); // obsolete precision parameter: read for
                                   // compatibility with old precision files
  class_read_double("k_scalar_step_sub",
                    ppr->k_step_sub); // obsolete precision parameter: read for
                                      // compatibility with old precision files
  class_read_double(
      "k_scalar_step_super",
      ppr->k_step_super); // obsolete precision parameter: read for
                          // compatibility with old precision files
  class_read_double(
      "k_scalar_step_transition",
      ppr->k_step_transition); // obsolete precision parameter: read for
                               // compatibility with old precision files
  class_read_double(
      "k_scalar_k_per_decade_for_pk",
      ppr->k_per_decade_for_pk); // obsolete precision parameter: read for
                                 // compatibility with old precision files
  class_read_double(
      "k_scalar_k_per_decade_for_bao",
      ppr->k_per_decade_for_bao); // obsolete precision parameter: read for
                                  // compatibility with old precision files
  class_read_double(
      "k_scalar_bao_center",
      ppr->k_bao_center); // obsolete precision parameter: read for
                          // compatibility with old precision files
  class_read_double("k_scalar_bao_width",
                    ppr->k_bao_width); // obsolete precision parameter: read for
                                       // compatibility with old precision files
  ...
  class_read_double("start_small_k_at_tau_c_over_tau_h",
                    ppr->start_small_k_at_tau_c_over_tau_h);
  class_read_double("start_large_k_at_tau_h_over_tau_k",
                    ppr->start_large_k_at_tau_h_over_tau_k);
  class_read_double("tight_coupling_trigger_tau_c_over_tau_h",
                    ppr->tight_coupling_trigger_tau_c_over_tau_h);
  class_read_double("tight_coupling_trigger_tau_c_over_tau_k",
                    ppr->tight_coupling_trigger_tau_c_over_tau_k);
  class_read_double("start_sources_at_tau_c_over_tau_h",
                    ppr->start_sources_at_tau_c_over_tau_h);

  class_read_int("tight_coupling_approximation",
                 ppr->tight_coupling_approximation);

  if (ppt->has_tensors == _TRUE_) {
    /** - ---> Include ur and ncdm shear in tensor computation? */
    class_call(
        parser_read_string(pfc, "tensor method", &string1, &flag1, errmsg),
        errmsg, errmsg);
    if (flag1 == _TRUE_) {
  ...
  class_read_double("perturb_integration_stepsize",
  ...
  class_read_double("perturb_sampling_stepsize",
                    ppr->perturb_sampling_stepsize);

  class_read_int("radiation_streaming_approximation",
  ...
  class_read_double("ur_fluid_trigger_tau_over_tau_k",
                    ppr->ur_fluid_trigger_tau_over_tau_k);
  class_read_double("ncdm_fluid_trigger_tau_over_tau_k",
                    ppr->ncdm_fluid_trigger_tau_over_tau_k);
  class_read_double("dncdm_fluid_trigger_tau_over_tau_k",
                    ppr->dncdm_fluid_trigger_tau_over_tau_k);

  class_test(ppr->ur_fluid_trigger_tau_over_tau_k ==
                 ppr->radiation_streaming_trigger_tau_over_tau_k,
             errmsg,
             "please choose different values for precision parameters "
             "ur_fluid_trigger_tau_over_tau_k and "
             "radiation_streaming_trigger_tau_over_tau_k, in order to avoid "
             "switching two approximation schemes at the same time");
  ...
  if (pba->N_ncdm > 0) {
    ...
    class_test(ppr->ncdm_fluid_trigger_tau_over_tau_k ==
               ...
               errmsg,
               "please choose different values for precision parameters "
               "ncdm_fluid_trigger_tau_over_tau_k and "
               "radiation_streaming_trigger_tau_over_tau_k, in order to avoid "
               "switching two approximation schemes at the same time");
    ...
    class_test(ppr->ncdm_fluid_trigger_tau_over_tau_k ==
               ...
               "please choose different values for precision parameters "
               "ncdm_fluid_trigger_tau_over_tau_k and "
               "ur_fluid_trigger_tau_over_tau_k, in order to avoid switching "
               "two approximation schemes at the same time");
  }

  class_read_double("neglect_CMB_sources_below_visibility",
  ...
  class_read_double("primordial_inflation_ratio_min",
                    ppr->primordial_inflation_ratio_min);
  class_read_double("primordial_inflation_ratio_max",
                    ppr->primordial_inflation_ratio_max);
  class_read_int("primordial_inflation_phi_ini_maxit",
                 ppr->primordial_inflation_phi_ini_maxit);
  class_read_double("primordial_inflation_pt_stepsize",
                    ppr->primordial_inflation_pt_stepsize);
  class_read_double("primordial_inflation_bg_stepsize",
                    ppr->primordial_inflation_bg_stepsize);
  class_read_double("primordial_inflation_tol_integration",
  ...
  class_read_int("primordial_inflation_attractor_maxit",
                 ppr->primordial_inflation_attractor_maxit);
  class_read_double("primordial_inflation_tol_curvature",
                    ppr->primordial_inflation_tol_curvature);
  class_read_double("primordial_inflation_aH_ini_target",
                    ppr->primordial_inflation_aH_ini_target);
  class_read_double("primordial_inflation_end_dphi",
                    ppr->primordial_inflation_end_dphi);
  class_read_double("primordial_inflation_end_logstep",
                    ppr->primordial_inflation_end_logstep);
  class_read_double("primordial_inflation_small_epsilon",
                    ppr->primordial_inflation_small_epsilon);
  class_read_double("primordial_inflation_small_epsilon_tol",
                    ppr->primordial_inflation_small_epsilon_tol);
  class_read_double("primordial_inflation_extra_efolds",
  ...
  class_read_double("hyper_sampling_curved_low_nu",
                    ppr->hyper_sampling_curved_low_nu);
  class_read_double("hyper_sampling_curved_high_nu",
                    ppr->hyper_sampling_curved_high_nu);
  ...
  class_read_double("hyper_flat_approximation_nu",
  ...
  class_read_double("k_step_trans_scalars",
                    ppr->q_linstep); // obsolete precision parameter: read for
                                     // compatibility with old precision files
  class_read_double("k_step_trans_tensors",
                    ppr->q_linstep); // obsolete precision parameter: read for
                                     // compatibility with old precision files
  class_read_double("k_step_trans",
                    ppr->q_linstep); // obsolete precision parameter: read for
                                     // compatibility with old precision files
  class_read_double("q_linstep_trans",
                    ppr->q_linstep); // obsolete precision parameter: read for
                                     // compatibility with old precision files
  class_read_double(
      "q_logstep_trans",
      ppr->q_logstep_spline); // obsolete precision parameter: read for
                              // compatibility with old precision files

  class_read_double("transfer_neglect_delta_k_S_t0",
                    ppr->transfer_neglect_delta_k_S_t0);
  class_read_double("transfer_neglect_delta_k_S_t1",
                    ppr->transfer_neglect_delta_k_S_t1);
  class_read_double("transfer_neglect_delta_k_S_t2",
                    ppr->transfer_neglect_delta_k_S_t2);
  class_read_double("transfer_neglect_delta_k_S_e",
                    ppr->transfer_neglect_delta_k_S_e);
  class_read_double("transfer_neglect_delta_k_V_t1",
                    ppr->transfer_neglect_delta_k_V_t1);
  class_read_double("transfer_neglect_delta_k_V_t2",
                    ppr->transfer_neglect_delta_k_V_t2);
  class_read_double("transfer_neglect_delta_k_V_e",
                    ppr->transfer_neglect_delta_k_V_e);
  class_read_double("transfer_neglect_delta_k_V_b",
                    ppr->transfer_neglect_delta_k_V_b);
  class_read_double("transfer_neglect_delta_k_T_t2",
                    ppr->transfer_neglect_delta_k_T_t2);
  class_read_double("transfer_neglect_delta_k_T_e",
                    ppr->transfer_neglect_delta_k_T_e);
  class_read_double("transfer_neglect_delta_k_T_b",
                    ppr->transfer_neglect_delta_k_T_b);

  class_read_double("transfer_neglect_late_source",
                    ppr->transfer_neglect_late_source);

  class_read_double("l_switch_limber", ppr->l_switch_limber);

  class_call(parser_read_string(pfc, "l_switch_limber_for_cl_density_over_z",
                                &string1, &flag1, errmsg),
             errmsg, errmsg);
  ...
  class_test(
      ...
      "l_switch_limber_for_cl_density_over_z. This syntax is deprecated since "
      "v2.5.0. Please use instead the two precision parameters "
      "l_switch_limber_for_nc_local_over_z, l_switch_limber_for_nc_los_over_z, "
      "defined in include/common.h, and allowing for better performance.");

  class_read_double("l_switch_limber_for_nc_local_over_z",
                    ppr->l_switch_limber_for_nc_local_over_z);
  class_read_double("l_switch_limber_for_nc_los_over_z",
                    ppr->l_switch_limber_for_nc_los_over_z);
  ...
  class_read_double("selection_sampling_bessel",
                    ppr->selection_sampling_bessel);
  class_read_double("selection_sampling_bessel_los",
  ...
  class_call(
      parser_read_string(pfc, "write background", &string1, &flag1, errmsg),
      errmsg, errmsg);
  ...
  class_call(
      parser_read_string(pfc, "write thermodynamics", &string1, &flag1, errmsg),
      errmsg, errmsg);
  ...
  class_call(parser_read_list_of_doubles(pfc, "k_output_values", &(int1),
                                         &(pointer1), &flag1, errmsg),
             errmsg, errmsg);

  if (flag1 == _TRUE_) {
    class_test(int1 > _MAX_NUMBER_OF_K_FILES_, errmsg,
               "you want to write some output for %d different values of k, "
               "hence you should increase _MAX_NUMBER_OF_K_FILES_ in "
               "include/perturbations.h to at least this number",
               int1);
    ...
    qsort(ppt->k_output_values, ppt->k_output_values_num, sizeof(double),
          compare_doubles);
  ...
  class_call(
      parser_read_string(pfc, "write primordial", &string1, &flag1, errmsg),
      errmsg, errmsg);
  ...
  if ((pnl->method == nl_halofit) && (pba->Omega0_fld != 0.) &&
      (pba->wa_fld != 0.))
  ...
  if (pnl->has_pk_eq == _TRUE_) {
    ...
    if (input_verbose > 0) {
      printf(" -> since you want to use Halofit with a non-zero wa_fld, "
             "calling background module to\n");
      printf("    extract the effective w(tau), Omega_m(tau) parameters "
             "required by the Pk_equal method\n");
    }
 ...
 */
int input_default_params(struct background *pba, struct thermo *pth,
                         struct perturbs *ppt, struct transfers *ptr,
                         struct primordial *ppm, struct spectra *psp,
                         struct nonlinear *pnl, struct lensing *ple,
                         struct output *pop) {
  ...
  /** Define all default parameter values (for input parameters) for each
   * structure:*/
  ...
  // MZ: initial conditions are as multiplicative factors of the radiation
  // attractor values
  pba->phi_ini_scf = 1;
  ...
  pba->K = 0.;
  pba->sgnK = 0;
  pba->Omega0_lambda = 1. - pba->Omega0_k - pba->Omega0_g - pba->Omega0_ur -
...
int input_default_precision(struct precision *ppr) {
  ...
  ppr->recfast_x_He0_trigger =
      0.995; /* raised from 0.99 to 0.995 for smoother Helium */
  ppr->recfast_x_He0_trigger2 =
      0.995; /* raised from 0.985 to same as previous one for smoother Helium */
  ppr->recfast_x_He0_trigger_delta = 0.05; /* found to be OK on 3.09.10 */
  ppr->recfast_x_H0_trigger =
      0.995; /* raised from 0.99 to 0.995 for smoother Hydrogen */
  ppr->recfast_x_H0_trigger2 = 0.995; /* raised from 0.98 to same as previous
  ...
  ppr->k_max_tau0_over_l_max =
      2.4; // very relevant for accuracy of lensed ClTT at highest l's
  ppr->k_step_sub = 0.05;
  ...
  ppr->start_small_k_at_tau_c_over_tau_h =
      0.0015; /* decrease to start earlier in time */
  ppr->start_large_k_at_tau_h_over_tau_k =
      0.07; /* decrease to start earlier in time */
  ppr->tight_coupling_trigger_tau_c_over_tau_h =
      0.015; /* decrease to switch off earlier in time */
  ppr->tight_coupling_trigger_tau_c_over_tau_k =
      0.01; /* decrease to switch off earlier in time */
  ppr->start_sources_at_tau_c_over_tau_h =
      0.008; /* decrease to start earlier in time */
  ...
  ppr->curvature_ini =
      1.; /* initial curvature; used to fix adiabatic initial conditions; must
             remain fixed to one as long as the primordial adiabatic spectrum
             stands for the curvature power spectrum */
  ppr->entropy_ini =
      1.; /* initial entropy; used to fix isocurvature initial conditions; must
             remain fixed to one as long as the primordial isocurvature spectrum
             stands for an entropy power spectrum */
  // ppr->gw_ini=0.25; /* to match normalization convention for GW in most of
  // literature and ensure standard definition of r */
  ppr->gw_ini = 1.;
  ...
  // We make the default approx switching threshold different from the above,
  // b/c CLASS cannot switch two approximations at once; This is handy if the
  // model includes both DNCDM and NCDM (i.e. massive neutrinos).
  ...
  ppr->hyper_sampling_curved_low_nu =
      7.0; // changed from 6.0 to 7.0 in v2.6.0, otherwise C2 can be very wrong
  ...
  class_test(ppr->smallest_allowed_variation < 0, ppr->error_message,
...
int input_fzerofun_1d(double input, void *pfzw, double *output,
                      ErrorMsg error_message) {

  class_call(
      input_try_unknown_parameters(&input, 1, pfzw, output, error_message),
      error_message, error_message);

  return _SUCCESS_;
}

int class_fzero_ridder(int (*func)(double x, void *param, double *y,
                                   ErrorMsg error_message),
                       double x1, double x2, double xtol, void *param,
                       double *Fx1, double *Fx2, double *xzero, int *fevals,
                       ErrorMsg error_message) {
  ...
  */
  ...
  if ((Fx1 != NULL) && (Fx2 != NULL)) {
    ...
    class_call((*func)(x1, param, &fl, error_message), error_message,
               error_message);
    class_call((*func)(x2, param, &fh, error_message), error_message,
               error_message);
  ...
  if ((fl > 0.0 && fh < 0.0) || (fl < 0.0 && fh > 0.0)) {
    ...
    for (j = 1; j <= MAXIT; j++) {
      xm = 0.5 * (xl + xh);
      class_call((*func)(xm, param, &fm, error_message), error_message,
                 error_message);
      *fevals = (*fevals) + 1;
      s = sqrt(fm * fm - fl * fh);
      if (s == 0.0) {
      ...
      ans = xnew;
      class_call((*func)(ans, param, &fnew, error_message), error_message,
                 error_message);
      *fevals = (*fevals) + 1;
      if (fnew == 0.0) {
        *xzero = ans;
...
int input_try_unknown_parameters(double *unknown_parameter,
                                 int unknown_parameters_size, void *voidpfzw,
                                 double *output, ErrorMsg errmsg) {
  /** Summary:
   * - Call the structures*/
  ...
  for (i = 0; i < unknown_parameters_size; i++) {
    sprintf(pfzw->fc.value[pfzw->unknown_parameters_index[i]], "%e",
            unknown_parameter[i]);
  }

  class_call(input_read_parameters(&(pfzw->fc), &pr, &ba, &th, &pt, &tr, &pm,
                                   &sp, &nl, &le, &op, errmsg),
             errmsg, errmsg);

  class_call(
      parser_read_int(&(pfzw->fc), "input_verbose", &param, &flag, errmsg),
      errmsg, errmsg);
  ...
  if (pfzw->required_computation_stage >= cs_nonlinear) {
    ...
    class_call(nonlinear_init(&pr, &ba, &th, &pt, &pm, &nl), nl.error_message,
               errmsg);
  }
  ...
  if (pfzw->required_computation_stage >= cs_transfer) {
    ...
    class_call(transfer_init(&pr, &ba, &th, &pt, &nl, &tr), tr.error_message,
               errmsg);
  }
  ...
  if (pfzw->required_computation_stage >= cs_spectra) {
    ...
    class_call(spectra_init(&pr, &ba, &pt, &pm, &nl, &tr, &sp),
               sp.error_message, errmsg);
  }
  ...
  for (i = 0; i < pfzw->target_size; i++) {
    ...
    case theta_s:
      ...
      break;
    case Omega_dcdmdr:
      rho_dcdm_today = ba.background_table[(ba.bt_size - 1) * ba.bg_size +
                                           ba.index_bg_rho_dcdm];
      if (ba.has_dr == _TRUE_)
        rho_dr_today = ba.background_table[(ba.bt_size - 1) * ba.bg_size +
                                           ba.index_bg_rho_dr];
      else
        rho_dr_today = 0.;
      output[i] = (rho_dcdm_today + rho_dr_today) / (ba.H0 * ba.H0) -
                  pfzw->target_value[i];
      break;
    case omega_dcdmdr:
      rho_dcdm_today = ba.background_table[(ba.bt_size - 1) * ba.bg_size +
      ...
      if (ba.has_dr == _TRUE_)
        ...
        rho_dr_today = 0.;
      output[i] = (rho_dcdm_today + rho_dr_today) / (ba.H0 * ba.H0) -
                  pfzw->target_value[i] / ba.h / ba.h;
      break;
    case Omega_scf:
      /** - In case scalar field is used to fill, pba->Omega0_scf is not equal
       * to pfzw->target_value[i].*/
      output[i] = ba.background_table[(ba.bt_size - 1) * ba.bg_size +
                                      ba.index_bg_rho_scf] /
                      (ba.H0 * ba.H0) -
                  ba.Omega0_scf;
      break;
    ...
    case omega_ini_dcdm:
      ...
      if (ba.has_dr == _TRUE_)
        ...
        rho_dr_today = 0.;
      output[i] =
          -(rho_dcdm_today + rho_dr_today) / (ba.H0 * ba.H0) + ba.Omega0_dcdmdr;
      break;
    case sigma8:
...
int input_get_guess(double *xguess, double *dxdy,
                    struct fzerofun_workspace *pfzw, ErrorMsg errmsg) {
  ...
  for (index_guess = 0; index_guess < pfzw->target_size; index_guess++) {
    ...
    case theta_s:
      xguess[index_guess] = 3.54 * pow(pfzw->target_value[index_guess], 2) -
                            5.455 * pfzw->target_value[index_guess] + 2.548;
    ...
    case Omega_dcdmdr:
      ...
      /* This formula is exact in a Matter + Lambda Universe, but only
         ...
         dxdy[index_guess]
         = 1.0;//exp(2./3.*ba.Gamma_dcdm/ba.H0*atanh(sqrt_one_minus_M)/sqrt_one_minus_M);
      */
    ...
    case omega_dcdmdr:
      ...
      */
      ...
      if (gamma < 1)
        ...
        a_decay = pow(1 + (gamma * gamma - 1.) / Omega_M, -1. / 3.);
      xguess[index_guess] =
          pfzw->target_value[index_guess] / ba.h / ba.h / a_decay;
      dxdy[index_guess] = 1. / a_decay / ba.h / ba.h;
      // printf("x = Omega_ini_guess = %g, dxdy = %g\n",*xguess,*dxdy);
      break;
    case Omega_scf:

      /** - This guess is arbitrary, something nice using WKB should be
       * implemented.
       *
       * - Version 2: use a fit: `xguess[index_guess]
       * = 1.77835*pow(ba.Omega0_scf,-2./7.); dxdy[index_guess] =
       * -0.5081*pow(ba.Omega0_scf,-9./7.)`;
       *
...
int input_find_root(double *xzero, int *fevals, struct fzerofun_workspace *pfzw,
                    ErrorMsg errmsg) {
  ...
  /** - Find root using Ridders method. (Exchange for bisection if you are
   * old-school.)*/
  class_call(class_fzero_ridder(input_fzerofun_1d, x1, x2,
                                1e-5 * MAX(fabs(x1), fabs(x2)), pfzw, &f1, &f2,
                                xzero, fevals, errmsg),
             errmsg, errmsg);
...
int input_auxillary_target_conditions(struct file_content *pfc,
                                      ...
                                      double target_value, int *aux_flag,
                                      ErrorMsg errmsg) {
  *aux_flag = _TRUE_;
  /*
  double param1;
...
int input_prepare_pk_eq(struct precision *ppr, struct background *pba,
                        struct thermo *pth, struct nonlinear *pnl,
                        int input_verbose, ErrorMsg errmsg) {
  ...
  /** - store the true cosmological parameters (w0, wa) somwhere before using
   * temporarily some fake ones in this function */
  ...
  /** - the fake calls of the background and thermodynamics module will be done
   * in non-verbose mode */
  ...
  class_alloc(pnl->pk_eq_w_and_Omega,
              pnl->pk_eq_tau_size * pnl->pk_eq_size * sizeof(double), errmsg);
  ...
  for (index_pk_eq_z = 0; index_pk_eq_z < pnl->pk_eq_tau_size;
       index_pk_eq_z++) {
    z[index_pk_eq_z] = exp(log(1. + ppr->pk_eq_z_max) /
                           (pnl->pk_eq_tau_size - 1) * index_pk_eq_z) -
                       1.;
  ...
  for (index_pk_eq_z = 0; index_pk_eq_z < pnl->pk_eq_tau_size;
    ...
    /* launch iterations in order to coverge to effective model with wa=0 but
     * the same chi = (tau[z_i] - tau_rec) */
    ...
    do {
      ...
      class_call(background_tau_of_z(pba, z[index_pk_eq_z], &tau_of_z),
                 pba->error_message, errmsg);
      class_call(thermodynamics_init(ppr, pba, pth), pth->error_message,
                 errmsg);
    ...
    /* Equivalent model found. Store w0(z) in that model. Find Omega_m(z) in
     * that model and store it. */

    pnl->pk_eq_w_and_Omega[pnl->pk_eq_size * index_pk_eq_z +
                           pnl->index_pk_eq_w] = pba->w0_fld;

    class_alloc(pvecback, pba->bg_size * sizeof(double), pba->error_message);
    class_call(background_at_tau(pba, tau_of_z, pba->long_info,
                                 pba->inter_normal, &last_index, pvecback),
               pba->error_message, errmsg);
    pnl->pk_eq_w_and_Omega[pnl->pk_eq_size * index_pk_eq_z +
                           pnl->index_pk_eq_Omega_m] =
  ...
  /** - restore cosmological parameters (w0, wa) to their true values before
   * main call to CLASS modules */
  ...
  if (input_verbose > 1) {
    ...
    for (index_pk_eq_z = 0; index_pk_eq_z < pnl->pk_eq_tau_size;
      ...
      fprintf(stdout, "    * at z=%e, tau=%e w=%e Omega_m=%e\n",
              z[index_pk_eq_z], pnl->pk_eq_tau[index_pk_eq_z],
              pnl->pk_eq_w_and_Omega[pnl->pk_eq_size * index_pk_eq_z +
                                     pnl->index_pk_eq_w],
              pnl->pk_eq_w_and_Omega[pnl->pk_eq_size * index_pk_eq_z +
                                     pnl->index_pk_eq_Omega_m]);
  ...
  class_call(array_spline_table_lines(pnl->pk_eq_tau, pnl->pk_eq_tau_size,
                                      pnl->pk_eq_w_and_Omega, pnl->pk_eq_size,
                                      pnl->pk_eq_ddw_and_ddOmega,
                                      _SPLINE_NATURAL_, errmsg),
             errmsg, errmsg);
